...
func (d *db) Update(ctx context.Context, user user.User) error {
	...
	filter := bson.M{"_id": objectID} //какое поле:чему равно, ищем определенного пользователя с определенным идентификатором
	...
	delete(updateUserObj, "_id") //нужно удалить (идентификатор в юзере) иначе он обновиться в базе
	...
	if result.MatchedCount == 0 { //монго ищет, потом обновляет. если у нас ноль - значит ничего не нашли (нет такого пользователя)
...
func (d *db) Delete(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("failde to convert user ID to ObjectID, ID=%s", id) /////
	}
